(function(){
  ...
  async function load(){
    ...
    try{
      ...
      if(page==='news.html')renderNews(target,d.news||[]);
  ...
  function renderNews(target,arr){
    if(!arr.length){target.innerHTML='<div class="content-empty">Nessuna news pubblicata al momento.</div>';return;}
    target.innerHTML=`<div class="content-grid">${arr.map(n=>`<article class="content-card">${n.image?`<div class="content-image" style="background-image:url('${esc(n.image)}')"></div>`:''}<div class="content-body"><small>${formatDate(n.date)}</small><h3>${esc(n.title||'News')}</h3>${n.excerpt?`<p>${esc(n.excerpt)}</p>`:''}${n.body?`<details><summary style="cursor:pointer;font-weight:900;color:#ffd400">Leggi la news</summary><p style="white-space:pre-line">${esc(n.body)}</p></details>`:''}</div></article>`).join('')}</div>`;
  }
