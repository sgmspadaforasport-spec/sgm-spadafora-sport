(function(){
  ...
  css.textContent=`
    ...
    .sponsor-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:24px;align-items:stretch}
    .sponsor-card{position:relative;min-height:245px;padding:30px 24px 24px;border:1px solid #e5e5e5;border-radius:22px;background:linear-gradient(180deg,#fff 0%,#fafafa 100%);box-shadow:0 16px 40px rgba(0,0,0,.07);display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;overflow:hidden;transition:transform .22s ease,box-shadow .22s ease,border-color .22s ease}
    .sponsor-card:before{content:"";position:absolute;inset:0 0 auto;height:5px;background:linear-gradient(90deg,#ffd400,#f1c400)}
    .sponsor-card:after{content:"PARTNER SGM";position:absolute;top:16px;right:18px;font-size:8px;font-weight:900;letter-spacing:1.2px;color:#b09a20}
    .sponsor-card:hover{transform:translateY(-4px);box-shadow:0 22px 54px rgba(0,0,0,.11);border-color:#d8c45a}
    .sponsor-logo-wrap{width:100%;min-height:150px;padding:20px;border-radius:16px;background:#fff;border:1px solid #ededed;display:flex;align-items:center;justify-content:center;box-shadow:inset 0 0 0 1px rgba(0,0,0,.01)}
    .sponsor-card img{display:block;max-width:88%;width:auto;max-height:108px;height:auto;object-fit:contain;filter:drop-shadow(0 5px 12px rgba(0,0,0,.05))}
    .sponsor-card strong{margin-top:18px;color:#171717;font-size:14px;line-height:1.25;font-weight:900;letter-spacing:.25px;text-transform:uppercase}
    .sponsor-card .sponsor-no-logo{font-size:34px;font-weight:900;color:#d4b400;letter-spacing:-1px}
    ...
    @media(max-width:1050px){.sponsor-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}
    @media(max-width:850px){.content-grid{grid-template-columns:repeat(2,1fr)}.sponsor-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}}
    @media(max-width:560px){.content-grid,.sponsor-grid{grid-template-columns:1fr}.gallery-grid{columns:1}.sponsor-card{min-height:220px;padding:28px 20px 22px}.sponsor-logo-wrap{min-height:135px}}
  ...
  function renderSponsors(target,arr){
    ...
    if(!items.length){target.innerHTML='<div class="content-empty">Nessuno sponsor pubblicato al momento.</div>';return;}
    target.innerHTML=`<div class="sponsor-grid">${items.map(s=>`<article class="sponsor-card"><div class="sponsor-logo-wrap">${s.image?`<img src="${esc(s.image)}" alt="${esc(s.name||'Sponsor')}">`:`<div class="sponsor-no-logo">SGM</div>`}</div><strong>${esc(s.name||'Sponsor')}</strong></article>`).join('')}</div>`;
  }
