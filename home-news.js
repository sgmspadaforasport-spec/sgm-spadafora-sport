(function(){
  ...
  function installStyle(){
    ...
    style.textContent=`
      ...
      @media(max-width:620px){
        .home-news-preview{padding:42px 0}
        #homeNewsDynamic{display:flex;overflow-x:auto;gap:14px;padding-bottom:6px;scroll-snap-type:x mandatory}
        #homeNewsDynamic .home-feature-news{flex:0 0 84%;scroll-snap-align:start}
        #homeNewsDynamic .home-feature-news-image{height:150px;min-height:150px}
      }
    `;
  ...
  async function init(){
    try{
      ...
      const combined=[...officialNews,...normalNews]
        .sort((a,b)=>{
      ...
      box.innerHTML=combined.map(n=>{
        ...
        return `<article class="home-feature-news">${n.image?`<div class="home-feature-news-image"><img src="${esc(n.image)}" alt="${esc(n.title||'News')}">${isOfficial?'<span>COMUNICATO UFFICIALE</span>':''}</div>`:''}<div class="home-feature-news-body"><small>${esc(label)}</small><h3>${esc(n.title||'News')}</h3><p>${esc(n.excerpt||n.body||'')}</p><a class="btn btn-primary" href="${href}">${button}</a></div></article>`;
      }).join('');
    }catch(e){
      console.warn('News Home non disponibili',e);
    }
  }
